...
MPV owns decoding, scaling, fullscreen rendering, keyboard input, and window-close handling. Python retains album selection, newest-first rotation, new-download FIFO priority, one-compressed-file read-ahead, display timing, retry policy, control-panel settings, and network/status overlays. Folder changes and newly queued photos are applied only at photo boundaries. This removes Pygame, SDL build requirements, Pillow display-sized pixel buffers, and the former decoded-image preload worker from normal playback.
