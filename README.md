...
Startup restores the folder selected through the control panel, or selects **All** when no folder was saved or that folder no longer exists. Loose photos in either root and photos more than one folder deep are ignored. Existing flat-cache photos stay on disk but no longer play; move the photos into Drive albums and run sync to populate the new cache layout. Empty Drive albums also get local directories and appear in the selector.
...
Every sync fetches all listing pages for the parent and its immediate albums, using one Google Drive connection for that pass. JPEG, PNG, WebP, HEIC, and HEIF sources are accepted. Each download is validated against its Drive checksum and size before publication. Images larger than the configured 1600×900 bounding box are EXIF-transposed and reduced with high-quality proportional resampling; they are never stretched or enlarged. A 2400×1350 landscape becomes 1600×900, while a 1000×2000 portrait becomes 450×900. Oversized ordinary JPEGs use quality 85, while HEIC/HEIF-to-JPEG derivatives retain quality 75; both use optimized progressive output. Oversized PNG and WebP files retain their formats. Files already within the limit remain byte-for-byte unchanged, so a small or already-pixelated JPEG is never recompressed merely for being cached. HEIC/HEIF sources are the exception because they must always become same-stem JPEGs; MPV never receives an iPhone source format. A JPEG/HEIC same-stem collision gets a deterministic suffix.

The `.photos.json` manifest records the last successful remote catalog, cache-processing profile, and each published file's SHA-256. Reuse requires matching source identity, the current dimension/quality profile, and a fresh cache hash. Changing a limit therefore causes one safe reprocessing pass instead of retaining stale oversized derivatives. A missing or corrupt manifest is rebuilt from Drive metadata and verified source bytes. Renames and moves reuse only verified bytes. Temporary hard links avoid copying full images while filenames are swapped, and abandoned staging directories are cleaned up on a later sync.
...
Changes apply starting with the next successfully displayed photo. The current photo finishes its original interval. Accepted changes update `DISPLAY_SECONDS` and `SELECTED_FOLDER` in the board-local `.env` without replacing its other settings. The next application start—including an automatic restart after an update—restores them. A saved folder that no longer exists falls back to All and updates `.env` accordingly. The `.env` file remains ignored by Git and is not touched by software updates. Refreshing the page displays the current setting. The panel and initial cloud sync start independently of playback. The panel also works in cache-only mode without internet access. The `sync` command does not start it.
...
The HTML panel has **Slideshow control**, **Wi-Fi control**, and **Notes** sections. With the board helper enabled, every launch starts a WPA2 setup hotspot after one bounded access-point scan, even when Netplan has already attached with saved credentials. The slideshow displays the hotspot name, setup password (`jamesbond`), and current control URL in a red multiline banner **without a timeout**, including when `CONTROL_URL_DISPLAY_SECONDS=0`. It stays through every photo and failed connection attempt until the user selects an access point and that connection succeeds. The setup password is separate from the home-Wi-Fi password. At the owner's request, the home-Wi-Fi password input is visible while typing; the credential is not echoed in responses or logs.
...
**Waiting for a choice means no Drive requests or automatic home-Wi-Fi retries.** A fresh helper-backed launch always opens setup mode; it never accepts a saved Netplan connection as the user's choice. Each listed BSSID remains a separate selection, which lets the user choose between physical access points sharing an SSID. After a user-selected connection succeeds, the helper checks it every 30 seconds with short request deadlines. Loss of that connection returns to setup mode, and a failed or timed-out attempt recreates the hotspot instead of retrying a saved network. Normal Drive sync resumes once after the selected connection succeeds and then uses its usual interval.
